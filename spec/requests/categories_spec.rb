...
DEFAULT_CATGORIES_SIZE = 3
CURRENT_USER_CATEGORYS_SIZE = 3
OTHER_USER_CATEGORYS_SIZE = 2
...
RSpec.describe 'Categories', type: :request do
  ...
  describe 'GET /categories' do
    before do
      create_list(:dynamic_category, CURRENT_USER_CATEGORYS_SIZE, user: current_user)
      create_list(:dynamic_category, OTHER_USER_CATEGORYS_SIZE, user: other_user)
    end
    ...
    it 'returns all categories belonging to current_user' do
      ...
      expect(json_response.size).to eq(CURRENT_USER_CATEGORYS_SIZE)
    ...
    it 'returns all categories belonging to current_user + default with user equal nil' do
      create_list(:category, DEFAULT_CATGORIES_SIZE) # with no user
      get CATEGORIES_URL, headers: headers

      expect(json_response.size).to eq(CURRENT_USER_CATEGORYS_SIZE + DEFAULT_CATGORIES_SIZE)
    end
  end
