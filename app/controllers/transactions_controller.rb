class TransactionsController < ApplicationController
    ...
    def index
        ...
        render_jsonapi_response(transactions, serializer: TransactionSerializer, include: [ :category, :currency ])
    end

    def show
        transaction = current_user.transactions.includes(:category, :currency).find(params[:id])

        render_jsonapi_response(transaction)
    end

    def create
        service = TransactionCreateService.new(current_user, transaction_params)

        render_jsonapi_response(service.call, status: :created)
    end
